...
# import requests
from rest_api.views import order
...
class Rest_Api_Order_Testing(TestCase):
    ...
    def setUp(self):
        script_dir = os.path.dirname(__file__)  # <-- absolute dir the script is inr
        rel_path = "order.json"
        abs_file_path = os.path.join(script_dir, rel_path)
        desc = open(abs_file_path, 'r')
        self.order = ast.literal_eval(desc.read())
        self.factory = APIRequestFactory(enforce_csrf_checks=True)
        self.url = 'http://127.0.0.0.1:8080/order/'

    def test_order_2_ways(self):
        r = self.factory.post('/order/', self.order, format='json')
        # using order view function to generate the responce
        response = order(r)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # def test_order_deserialization(self):
        #     # python requests are not working ???
